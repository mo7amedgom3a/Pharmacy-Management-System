...
async def create(supplier: SupplierCreate, session: AsyncSession = Depends(get_session)) -> Supplier:
    return await create_supplier(supplierDto=supplier, session=session)


# Update a supplier
@router.put("/{supplier_id}", response_model=Supplier)
async def update(supplier_id: int, supplier: Supplier, session: AsyncSession = Depends(get_session)) -> Supplier:
