...
router = APIRouter(prefix="/pharmacy", tags=["Pharmacy"], dependencies=[Depends(require_role(roles["admin"]))])
...
@router.get("/inventory/{pharmacy_id}", response_model=List[Inventory], status_code=200, dependencies=[Depends(require_role(roles["pharmacist"]))])
...
@router.get("/inventory/{pharmacy_id}/{drug_name}", response_model=Inventory, status_code=200, dependencies=[Depends(require_role(roles["pharmacist"]))])
...
@router.get("/billing/{pharmacy_id}", response_model=List[Billing], status_code=200, dependencies=[Depends(require_role(roles["pharmacist"]))])
...
@router.get("/employee/{pharmacy_id}", response_model=List[Employee], status_code=200)
